...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Numbers: ' + songScore + ' | Pepsi Cans Bought: ' + songMisses + ' | U Rappin: ' + ratingName;
		} else {
			scoreTxt.text = 'Numbers: ' + songScore + ' | Pepsi Cans Bought: ' + songMisses + ' | U Rappin: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
	...
	function openChartEditor()
	{
		...
		#if desktop
		DiscordClient.changePresence("Cheats Menu", null, null, true);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("THIS BITCH DEAD - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0) ratingFC = "Turn off Bot";
			if (goods > 0) ratingFC = "You good";
			if (bads > 0 || shits > 0) ratingFC = "Full Combo";
			if (songMisses > 0 && songMisses < 10) ratingFC = "Bitch";
